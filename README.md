...
> ⚠️ **先读这段**：任天堂**没有公开 API**，这里用的是「Nintendo Switch
> Parental Controls」手机 App 背后的接口。它**不是官方支持的用法**，任天堂
> 随时可能修改导致失效；也拿不到「这台机器总共玩了多少小时」的历史总量，
> 只能拿到最近的每日记录。**如果不想要这些麻烦，用上面的「截图识别」就够了。**

### 前提条件

1. 主机上已开启「家长控制」并绑定过手机 App（没开的话接口里没有任何数据）
2. 电脑能访问任天堂服务器；国内直连通常需要代理，
   可先设置环境变量：`set SW_PROXY=http://127.0.0.1:7890`

### 绑定步骤

1. 打开网页，点「同步游玩时长」→「任天堂账号」页签 →「获取登录链接」
2. 复制弹出的链接，在浏览器里打开，用任天堂账号登录
3. 登录后会停在 **Linking an External Account** 页面：
   **右键**「Select this person」按钮 → **复制链接地址**
   —— 千万不要直接点这个按钮，点了就拿不到令牌，得重来
4. 回到网页，把复制的**完整链接**粘贴进输入框 →「完成绑定」
   （链接形如 `npf54789befb391a838://auth#session_token_code=...`）
5. 绑定成功后选统计范围 →「开始同步」，结果同样进入确认表格
